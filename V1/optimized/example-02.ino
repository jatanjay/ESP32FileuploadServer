...
String listFiles(bool ishtml) {
  ...
  // Array containing files to be excluded
  String excludeFiles[] = {"Arrhythmia.text", "Norepi.txt", "pure-min.css", "styles.css"};
  ...
  while (foundfile) {
    bool exclude = false; // Flag to check if the file should be excluded
    for (int i = 0; i < sizeof(excludeFiles) / sizeof(excludeFiles[0]); i++) {
      if (String(foundfile.name()) == excludeFiles[i]) {
        exclude = true; // Found file in exclude list
